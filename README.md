...
The issue of climate change is becoming one of the biggest challenges our world is facing. There are fervent believers and deniers all over the world and it can be difficult for a government to understand how their people as a whole feel about climate change; traditional public survey methods are time and resource costly and often have a low participation rate. It is becoming an increasingly politicized topic, adding to this decisiveness. Understanding how the public opinion is leaning towards climate change can help governments and businesses make the best targeted movements and implement policies in areas where people will be supportive of these changes.

Social media is very widely used and people, as we all know, love to voice their opinions using these platforms. Social media can be a great tool to collect and analyze the public opinions of people. The purpose of this project is to use social media (Twitter, specifically) to uncover how people are feeling towards climate change. We believe that this information can inform government bodies as well as companies in the green sector to find their target audiences. We are curious to see if locations that have experienced a higher change in weather conditions are stronger believers in climate change. Congruently, also locations that have not experienced climage change are more likley to be deniers.
...
The main takeaway from our analysis is that our input data needs improving. The features present in our dataset are not interacting consistently to predict the stance of a user; neither, is any one factor on its own influencing this stance enough to be considered a consistent factor. In order for us to use social media tweets to find a user's stance on climate change we would have to introduce new factors that have a higher impact in order to optimize our model. Our data also has some biases; the dataset is mostly skewed towards believers in climate change. We also found that the majority of the tweets were male users. Moving forward, we could next attempt over- or under- sampling the data set to decrease the effect these biases are having and potentially optimize the model's performance. If the model can be optimized, we would like to refactor the code to scrape the data in real time from Twitter to have an up to date view of the sentiment on climate change. This would involve implementing some Natural Language Processing and could be achieved given more time and resources.

Despite our model not achieving optimal performance in this project, we believe social media could be a great tool for research into the opinions of people and this kind of research would help countries around the world. Social media is already being used to track opinions for marketing and it can have an even greater impact moving forward in our future's biggest issues as we learn to navigate the growing global population.
