...
Our biggest takeaway from this analysis is that no one or two features are directly impacting an individual's stance; the whole ecosystem of features are interacting. Every individual is so different it is difficult to predict an opinion; two people in the same location with the same features could feel vastly different about the subject of climate change. However, our data does have some biases; the dataset is mostly skewed towards believers in climate change and the majority of the tweets were male users. Moving forward, we could next attempt over- or under- sampling the data set to decrease the effect these biases are having and potentially further optimize the model's performance. If the model can be optimized, we would like to refactor the code to scrape the data in real time from Twitter to have an up to date view of the sentiment on climate change. This would involve implementing some Natural Language Processing and could be achieved given more time and resources.

We believe social media could be a great tool for research into the opinions of people and this kind of research would help countries around the world. Social media is already being used to track opinions for marketing and it can have an even greater impact moving forward in our future's biggest issues as we learn to navigate the growing global population.
